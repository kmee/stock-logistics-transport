...
class TestTMSDeliveryStopsDemo(TransactionCase):
    ...
    def test_demo_brazil_order_exists(self):
        """Test that Brazil demo order exists"""
        brazil_order = self.env.ref("tms_route_optimizer.demo_tms_order_brazil_1")
        self.assertTrue(brazil_order)
        self.assertEqual(brazil_order.name, "ORD-BRA-001")
        self.assertGreater(len(brazil_order.stop_ids), 0)

    def test_demo_brazil_stops_count(self):
        """Test that Brazil demo order has 6 stops"""
        brazil_order = self.env.ref("tms_route_optimizer.demo_tms_order_brazil_1")
        self.assertEqual(len(brazil_order.stop_ids), 6)
